<x-layouts.auth title="Giriş">
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-6">
            ...
            <h1 class="text-xl font-bold text-gray-800">TOPLUSMS</h1>
            <p class="text-gray-400 mt-1 text-sm">Hesabınıza giriş yapın</p>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 sm:p-8">
            <form method="POST" action="{{ route('login') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="email" name="email" placeholder="ornek@email.com" value="{{ old('email') }}" required autofocus
                           class="w-full px-3 py-2.5 border {{ $errors->has('email') ? 'border-red-300' : 'border-gray-300' }} rounded text-sm focus:outline-none focus:ring-1 focus:ring-[#2563eb] focus:border-[#2563eb]">
                ...
                <div>
                    ...
                    <input type="password" name="password" placeholder="••••••••" required
                           class="w-full px-3 py-2.5 border {{ $errors->has('password') ? 'border-red-300' : 'border-gray-300' }} rounded text-sm focus:outline-none focus:ring-1 focus:ring-[#2563eb] focus:border-[#2563eb]">
        ...
        <p class="text-center mt-5 text-sm text-gray-400">
            Hesabınız yok mu?
            <a href="{{ route('register') }}" class="text-[#2563eb] hover:text-[#1d4ed8] font-medium">Kayıt olun</a>
        </p>
    </div>
</x-layouts.auth>
