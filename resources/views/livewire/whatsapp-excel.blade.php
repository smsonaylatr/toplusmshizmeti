<div>
    <h1 class="text-2xl font-bold text-gray-800 mb-1">Excel ile WhatsApp Mesaj</h1>
    <div class="mb-5"><h2 class="text-base font-semibold text-green-600 border-b-2 border-green-600 inline-block pb-1">Excel Dosyasından Mesaj Gönder</h2></div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-4 bg-gradient-to-r from-green-500 to-green-600 text-white text-[12px] leading-relaxed flex items-start gap-3">
            ...
            <div>
                <strong>Excel ile WhatsApp Mesaj</strong><br>
                Excel dosyanızdaki numaralara toplu WhatsApp mesajı gönderin. Dosya formatı: ilk sütunda telefon numaraları olmalıdır.
            </div>
        </div>
            ...
            <form wire:submit="send">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        {{-- Dosya yükleme --}}
                        <div>
                            <label class="block text-xs text-gray-500 mb-1.5 font-medium">Excel Dosyası Seçin</label>
                            <div class="relative">
                                <input wire:model="excelFile" type="file" accept=".xlsx,.xls,.csv" class="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm bg-gray-50/50 focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all file:mr-4 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-green-50 file:text-green-700 hover:file:bg-green-100">
                            </div>
                            <p class="text-[11px] text-gray-400 mt-1">Desteklenen: .xlsx, .xls, .csv (Maks 5MB)</p>
                        </div>
                        @error('excelFile') <p class="text-red-500 text-xs">{{ $message }}</p> @enderror
                        ...
                        {{-- Örnek Format --}}
                        <div class="p-3 bg-blue-50 rounded-xl border border-blue-100 text-[11px] text-blue-700">
                            <strong>Excel Format Örneği:</strong>
                            <table class="mt-2 w-full text-[11px]">
                                <thead><tr class="border-b border-blue-200"><th class="text-left pb-1">Telefon</th><th class="text-left pb-1">İsim</th></tr></thead>
                                <tbody>
                                    <tr><td class="py-0.5">5301234567</td><td>Ahmet Yılmaz</td></tr>
                                    <tr><td class="py-0.5">5321234567</td><td>Mehmet Demir</td></tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <textarea wire:model.live="message" rows="12" placeholder="WhatsApp mesajınızı yazın..." class="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm resize-y bg-gray-50/50 focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all"></textarea>
                        </div>
                        <div class="text-[11px] text-gray-400">{{ $charCount }} karakter</div>
