<div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div><h1 class="text-2xl font-bold text-white tracking-tight">SMS Mesajları</h1><p class="text-sm text-gray-500 mt-0.5">Tüm SMS mesajlarını izleyin</p></div>
    </div>
    <div class="glass-card overflow-hidden">
        <div class="p-4 flex flex-col sm:flex-row gap-3" style="border-bottom: 1px solid var(--admin-border);">
            ...
            <select wire:model.live="statusFilter" class="admin-select w-full sm:w-auto">
                ...
                <option value="sent">Gönderildi</option><option value="delivered">İletildi</option><option value="failed">Başarısız</option><option value="pending">Bekliyor</option>
            </select>
        </div>
        <div class="overflow-x-auto">
            <table class="admin-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="text-gray-300 text-[13px]">{{ $msg->user?->name ?? '—' }}</td>
                        <td class="text-gray-400 text-[13px]">{{ $msg->recipient }}</td>
                        <td class="text-gray-500 text-[12px] max-w-[280px] truncate hidden md:table-cell">{{ $msg->message }}</td>
                        <td class="text-center">
                            <span class="{{ in_array($msg->status, ['sent','delivered']) ? 'status-success' : ($msg->status === 'failed' ? 'status-danger' : 'status-warning') }}">
                                {{ in_array($msg->status, ['sent','delivered']) ? 'Gönderildi' : ($msg->status === 'failed' ? 'Başarısız' : 'Bekliyor') }}
                            </span>
                        </td>
                        <td class="text-right text-[11px] text-gray-600">{{ $msg->created_at->format('d.m.Y H:i') }}</td>
                    </tr>
                    @empty
                    <tr><td colspan="5"><div class="text-center py-10"><p class="text-sm text-gray-600">Mesaj bulunamadı</p></div></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
